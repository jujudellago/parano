...
module Slugger
  ...
  class << self
    def escape(str)
      ss=str.gsub(/'+/i, "@") # initial commas to non ASCII char
      ss.gsub!(/!/,"tchick")
      begin
        s = Iconv.iconv('ascii//translit', 'utf-8', ss).to_s # transform UTF-8 chars to the ASCII equivalent or convination (example: ø => o, á => 'a)
      rescue  Iconv::IllegalSequence
        s= ss.downcase.gsub(/\s+/, '_').gsub(/[^a-zA-Z0-9_]+/, '')
      end
      s.gsub!(/'+/i, "") # remove new commas generated in the translit
      s.gsub!(/[\W_]+/, '-') # all non-word chars to dashes (including underscores)
      s.gsub!(/(^( |-)+)|(( |-)+$)/, '')            # remove initial and final espaces or dashes
      s.downcase!
      s
    end
  end
