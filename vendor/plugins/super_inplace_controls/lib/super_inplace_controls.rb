...
module Flvorful
  module SuperInplaceControls
    ...
    # These controls were designed to be used with habtm & hm associations.  Just use the 
    # object and the foreign key as the attribute (see examples).  You can specify a chain of methods to 
    ...
    # 
    # ==OPTIONS
    # <b>:final_text</b> - this is an array of methods calls or a special symbol to be executed after the update
    # ie: 
    ...
    # The method call will look like: 
    #   object.status.title
    #   
    # <em>special symbols</em>
    # :collection - Use this as your final text if you want to use a checkbox collection 
    # on a HABTM or HM association.  This will call a method stack similar to:
    #   object.collections.map { |e| e.title || e.name }.join (", ")
    # ie: 
    #   in_place_edit_for :product, :category_ids, {:final_text => :collection }
    #
    # <b>:highlight_endcolor</b> - the end color for the "highlight" visual effect. ie: 
    #   in_place_edit_for( :prospect, "status_id", { :final_text => ["status", "title"], highlight_endcolor => "'#ffffff'" })
    # 
    # <b>:highlight_startcolor</b> - the start color for the "highlight" visual effect. ie: 
    #   in_place_edit_for( :prospect, "status_id", { :final_text => ["status", "title"], :highlight_startcolor => "'#ffffff'" })
    # 
    # <b>:error_messages</b> - the error messages div that errors will be put into.  defaults to #error_messages - ie: 
    #   in_place_edit_for( :prospect, "status_id", { :error_messages => "my_error_div", :highlight_startcolor => "'#ffffff'" })
    # 
    # <b>:error_visual_effect</b> -The visual effect to use for displaying your error_message div.  Defaults to :slide_down. ie:
    #   in_place_edit_for( :prospect, "status_id", { :error_visual_effect => :appear })
    # 
    # ==TODO
    # <b>:final_text_operation</b> - this is a helper method that you want to call once the final_text is generated
    # ie: 
    #    in_place_edit_for( :prospect, "status_id", { :final_text => ["status", "title"] }, { :final_text_operation => ["truncate", 30] })
    # This will call truncate on the final_text and pass in 30 as the argument.
    # Everything after the first argument (the method to call) is sent as an argument to the method
    # The method call is similar to: truncate(<result of final_text>, 30)
    # 
    # 
    ...
    module HelperMethods
      ...
      def in_place_select(object, method, options = {})
        #RAILS_DEFAULT_LOGGER.error("\n in_place_select:  object #{object.inspect} , method=#{method} options=#{options} \n")
